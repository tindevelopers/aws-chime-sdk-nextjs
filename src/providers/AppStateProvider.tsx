...
export function AppStateProvider({ children }: Props) {
  ...
  // Use router.query for URL parameters in Next.js
  const [meetingId, setMeetingId] = useState('');
  const [region, setRegion] = useState('');
  const [meetingMode, setMeetingMode] = useState(MeetingMode.Attendee);
  
  // Initialize state from URL parameters when router is ready
  useEffect(() => {
    if (router.isReady) {
      if (router.query.meetingId && typeof router.query.meetingId === 'string') {
        setMeetingId(router.query.meetingId);
      }
      if (router.query.region && typeof router.query.region === 'string') {
